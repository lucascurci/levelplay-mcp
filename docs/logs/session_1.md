...
5. **`mcp.get_context()` doesn't exist** — Original code used `mcp.get_context().request_context["client"]` to access the shared httpx client inside tool functions. This crashed in production when Celeste called the tools: `AttributeError: 'FastMCP' object has no attribute 'get_context'`. Attempted Context injection (`ctx.lifespan_context`) but that attribute also doesn't exist in FastMCP 2.14.5. Ultimately dropped lifespan/Context entirely in session 2 — see `docs/logs/session_2.md`.
